...
column is called **MiniBus' Price**.

**Formula**

> `Price = MiniBus / TheOther`
...
However, it's interesting that their **cyclomatic complexity** per LOC is almost
the same in all comparisons, not following the behavior of **cognitive complexity**
per LOC.
...
`MiniBus` has three key differences compared to other packages:

1. it uses [Fluent Interfaces][3] as much as possible
2. it follows a functional paradigm as much as possible
3. it uses immutable values as much as possible, deferring the mutation as much 
   as possible.

p.s. **mutation is required at some point**, but it's done as late as possible.

All of that makes the logic more natural, simple and easy to understand.

p.s. fluent interfaces require **smart values**, like custom collections instead
of plain array values. But despite a little additional cyclomatic complexity
in the class where it's implemented, the overall cognitive complexity is much
smaller where it's used.
